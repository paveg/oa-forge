...
fn load_ts_config(path: &std::path::Path) -> Result<Config, String> {
    ...
    // Use JSON-escaped path to prevent JS injection via special characters in filenames
    let path_json = serde_json::to_string(&abs_path.display().to_string())
        .map_err(|e| format!("cannot serialize path: {e}"))?;
    let eval_script =
        format!("import c from {path_json}; process.stdout.write(JSON.stringify(c.default ?? c))");
...
/// Compute a deterministic content hash for incremental generation.
...
/// Sanitize a name for use as a filename, preventing path traversal.
...
/// The output path is passed as `$1` to avoid shell injection via path metacharacters.
fn run_after_write_hooks(hooks: &[String], output: &std::path::Path) {
    let output_str = output.display().to_string();
    for cmd in hooks {
        eprintln!("Running hook: {cmd} $1  (where $1 = {output_str})");
        match std::process::Command::new("sh")
            .args(["-c", cmd, "--", &output_str])
            .status()
        {
            Ok(status) if status.success() => {}
            Ok(status) => eprintln!("warn: hook exited with {status}: {cmd}"),
...
fn emit_client(
    ...
    match client_type {
        ClientType::Fetch => {
            oa_forge_emitter_client::emit(api, client_style, out).expect("client emitter failed")
        }
        ClientType::Axios => oa_forge_emitter_axios::emit(api, out).expect("axios emitter failed"),
        ClientType::Hono => oa_forge_emitter_hono::emit(api, out).expect("hono emitter failed"),
        ClientType::Angular => {
            oa_forge_emitter_angular::emit(api, out).expect("angular emitter failed")
        }
    }
}
...

    {
        ...
        rayon::scope(|s| {
            s.spawn(|_| {
                ...
                oa_forge_emitter_types::emit(&api, &mut out).expect("types emitter failed");
            ...
            if opts.hooks {
                s.spawn(|_| {
                    ...
                    oa_forge_emitter_query::emit_for(&api, &mut out, fw)
                        .expect("query emitter failed");
            ...
            if opts.zod {
                s.spawn(|_| {
                    let mut out = String::new();
                    oa_forge_emitter_zod::emit(&api, &mut out).expect("zod emitter failed");
            ...
            if opts.valibot {
                s.spawn(|_| {
                    let mut out = String::new();
                    oa_forge_emitter_valibot::emit(&api, &mut out).expect("valibot emitter failed");
            ...
            if opts.msw {
                s.spawn(|_| {
                    let mut out = String::new();
                    oa_forge_emitter_msw::emit(&api, &mut out).expect("msw emitter failed");
            ...
            if opts.mock {
                s.spawn(|_| {
                    let mut out = String::new();
                    oa_forge_emitter_mock::emit(&api, &mut out).expect("mock emitter failed");
    ...
    if opts.dry_run {
        ...
        if opts.split == SplitMode::Tag {
            ...
            for (tag, endpoints) in &tag_groups {
                let tag_slug = safe_filename(&tag.to_lowercase().replace(' ', "-"));
        ...
        if opts.split == SplitMode::Endpoint {
            ...
            for ep in &api.endpoints {
                ...
                files.push((
                    endpoints_dir.join(format!("{}.gen.ts", safe_filename(&ep.operation_id))),
                    ep_formatted,
                ));
            }
        }
...
fn run_generate(args: GenerateArgs, config: Config) -> Result<()> {
    ...
    let custom_client_name = args
        .custom_client_name
        .or(config.custom_client_name)
    ...
    let opts = GenerateOptions {
        ...
        split: args.split.or(config.split).unwrap_or(SplitMode::Single),
        query_framework: args
            .query_framework
            .or(config.query_framework)
            .unwrap_or(QueryFrameworkArg::React),
        client_style,
        custom_client_path,
