...
From the same `petstore.yaml` (default settings):

| | oa-forge | Orval v8.5 |
|---|---|---|
| **Default output** | 4 files (types, client, hooks, index) | 1 file (configurable via `mode: tags`) |
| **Lines** | ~200 | ~350 |
| **useQuery** | `useQuery` + `useSuspenseQuery` | `useQuery` + `useSuspenseQuery` |
| **queryOptions** | TanStack Query v5 `queryOptions()` helper | Available via config |
| **prefetchQuery** | Generated by default | Not generated by default |
| **Loader support** | `ensureQueryData` loader function | Not generated by default |
| **Error handling** | Typed `ApiError<T>` class | Error type via generics |
| **Custom client** | `onRequest` / `onResponse` interceptors | `mutator` pattern (custom Axios instance) |
| **queryKey** | operationId-based (`['listPets']`) | Path-based (`['/pets']`) |
| **Readability** | Clean, minimal generics | `Awaited<ReturnType<typeof ...>>` nesting |
